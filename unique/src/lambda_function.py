...
def unique_twitter():
    try:
        ...
        actions                 = []
        unique_resolved_id_list = []

        # confirm that resolved_id of item with Twitter tag is in unique_resolved_id_list
        for item_id in res_json["list"].keys():
            ...
            if len(res_json["list"][item_id]["tags"].keys()) == 1 and (res_json["list"][item_id]["resolved_id"] in unique_resolved_id_list):
                action = {
                    "action" : "delete",
                    "item_id": item_id
                }
                ...
                unique_resolved_id_list.append(res_json["list"][item_id]["resolved_id"])
